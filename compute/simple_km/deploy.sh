...
rsync -av -e ssh --exclude='temp/' --exclude='data/' --exclude='deploy.sh' $proj_root'/simple_km' $lyap:Hall/
rsync -av -e ssh --exclude='temp/' --exclude='data/' --exclude='deploy.sh' $proj_root'/simple_km' $poin:Hall/
